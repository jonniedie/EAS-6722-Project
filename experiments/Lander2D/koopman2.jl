...
# widths = [1, 20] #range_width.(requirements)
# g_single = (v,w) -> (v/w)^2
...
## Simulate
@time mc_sol = mc_solve(ODE_prob, ic, p, Tsit5(), EnsembleSerial(); trajectories=10000);
...
function running_expectation(trajectories)
    ...
    outs = meets_requirements.(mc_sol.u)
...
scatter!(vx, vy, markersize=1, color=:ivory, markerstrokewidth=0, legend=false, size=(600,400))
xlims!(-1.5, 1.5)
ylims!(-60, 0)
...
## Plot mc vs koopman
i = 200:200:200_000
plot(i, reduce(hcat, running_expectation(i[end])[i] for j in 1:20), label="Monte Carlo", primary=false)

# for j in 1:3
#     plot!(i, running_expectation(i[end])[i], label="mc$j")
# end
